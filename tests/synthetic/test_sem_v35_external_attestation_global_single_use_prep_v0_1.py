...
class SemV35ExternalAttestationGlobalSingleUsePrepTests(unittest.TestCase):
    ...
    def _bundle(self, root: Path):
        ...
        evidence = v35.build_external_evidence_reference_preview(
            authority_binding=binding, authority_descriptor=descriptor,
            evidence_path=str(evidence_path.resolve()), evidence_id="V35-EVIDENCE-001",
            expected_evidence_sha256=evidence_sha,
        )
        global_binding = v35.build_global_store_binding_preview(
            authority_binding=binding, authority_descriptor=descriptor,
            evidence_reference=evidence, global_store_binding_id="V35-GLOBAL-STORE-001",
        )
        return contract, external, store_root, profile, descriptor, binding, evidence_path, evidence, global_binding

    def test_v35_01_external_evidence_file_is_hash_bound_only(self):
        with tempfile.TemporaryDirectory() as tmp:
            *_, binding, _, evidence, _ = self._bundle(Path(tmp))[4:]
    ...
    def test_v35_02_wrong_evidence_hash_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            _, _, _, _, descriptor, binding, evidence_path, _, _ = self._bundle(Path(tmp))
            with self.assertRaises(PermissionError):
                v35.build_external_evidence_reference_preview(
                    authority_binding=binding, authority_descriptor=descriptor,
                    evidence_path=str(evidence_path), evidence_id="V35-EVIDENCE-002",
                    expected_evidence_sha256="d" * 64,
                )

    def test_v35_03_repo_local_evidence_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            _, _, _, _, descriptor, binding, _, _, _ = self._bundle(Path(tmp))
            repo_file = v35.ROOT / "README.md"
            if repo_file.exists():
                with self.assertRaises(PermissionError):
                    v35.build_external_evidence_reference_preview(
                        authority_binding=binding, authority_descriptor=descriptor,
                        evidence_path=str(repo_file), evidence_id="V35-EVIDENCE-002",
    ...
    def test_v35_04_evidence_missing_file_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(PermissionError):
                v35.build_external_evidence_reference_preview(
                    authority_binding=binding, authority_descriptor=descriptor,
                    evidence_path=str((Path(tmp) / "missing.txt").resolve()), evidence_id="V35-EVIDENCE-002",
                    expected_evidence_sha256="e" * 64,
                )

    def test_v35_05_evidence_mutation_after_binding_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            _, _, _, _, descriptor, binding, evidence_path, evidence, _ = self._bundle(Path(tmp))
            evidence_path.write_bytes(b"changed")
            with self.assertRaises(PermissionError):
                v35.validate_external_evidence_reference_preview(
                    evidence, authority_binding=binding, authority_descriptor=descriptor
                )

    def test_v35_06_evidence_unknown_field_rejected_after_rehash(self):
        with tempfile.TemporaryDirectory() as tmp:
            _, _, _, _, descriptor, binding, _, evidence, _ = self._bundle(Path(tmp))
            ...
            with self.assertRaises(PermissionError):
                v35.validate_external_evidence_reference_preview(
                    tampered, authority_binding=binding, authority_descriptor=descriptor
                )
    ...
    def test_v35_08_global_binding_pins_one_store_identity_structurally(self):
        with tempfile.TemporaryDirectory() as tmp:
            _, _, _, _, _, _, _, _, global_binding = self._bundle(Path(tmp))
    ...
    def test_v35_09_global_binding_exact_keyset_rejects_extra(self):
        with tempfile.TemporaryDirectory() as tmp:
            _, _, _, _, descriptor, binding, _, evidence, global_binding = self._bundle(Path(tmp))
    ...
    def test_v35_11_delete_rotation_and_single_use_claims_remain_false(self):
        with tempfile.TemporaryDirectory() as tmp:
            _, _, _, _, _, _, _, evidence, global_binding = self._bundle(Path(tmp))
    ...
    def test_v35_12_evidence_path_copy_does_not_attest_origin(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            _, _, _, _, descriptor, binding, evidence_path, _, _ = self._bundle(root)
            ...
            copied = v35.build_external_evidence_reference_preview(
                authority_binding=binding, authority_descriptor=descriptor,
                evidence_path=str(copy_path.resolve()), evidence_id="V35-EVIDENCE-COPY",
    ...
    def test_v35_13_alternate_store_can_still_form_separate_preview(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            contract, external, _, _, _, _, _, _, _ = self._bundle(root)
            ...
            self.assertNotEqual(binding2["bound_store_root_resolved"], str((root / "authority-store").resolve()))
    ...
    def test_v35_14_invalid_evidence_id_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(PermissionError):
                v35.build_external_evidence_reference_preview(
                    authority_binding=binding, authority_descriptor=descriptor,
                    evidence_path=str(evidence_path), evidence_id="../bad",
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(PermissionError):
                v35.build_global_store_binding_preview(
                    authority_binding=binding, authority_descriptor=descriptor,
                    evidence_reference=evidence, global_store_binding_id="../bad",
                )

    def test_v35_16_authority_descriptor_substitution_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            _, _, store_root, _, descriptor, binding, evidence_path, _, _ = self._bundle(root)
            other = v34.build_external_authority_descriptor_preview(
                ...
                trust_anchor_fingerprint_sha256=descriptor["trust_anchor_fingerprint_sha256"],
                authority_epoch="EPOCH-001",
            )
            with self.assertRaises(PermissionError):
                v35.build_external_evidence_reference_preview(
                    authority_binding=binding, authority_descriptor=other,
    ...
    def test_v35_18_report_remains_non_authorizing(self):
        ...
        for key in (
            ...
            "authorization_consumed", "model_run_authorized", "model_contact_authorized",
            "model_qualified",
        ):
