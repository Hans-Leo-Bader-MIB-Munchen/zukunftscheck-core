...
class MinistralPreflightOnlyGateTests(unittest.TestCase):
    def test_r01_live_gate_is_consumed_and_closed_after_one_preflight(self) -> None:
        ...
        self.assertFalse(payload["model_load_authorized"])
        self.assertFalse(payload["localhost_preflight_authorized"])
        self.assertFalse(payload["model_contact_authorized"])
        ...
        self.assertTrue(payload["preflight_contact_performed"])
        self.assertEqual(payload["observed_preflight_contact_count"], 1)
        self.assertEqual(payload["observed_generation_request_count"], 0)
        self.assertEqual(payload["preflight_outcome"], "EXECUTED_ONCE_FAILED_IDENTITY_BINDING")
        self.assertFalse(payload["model_qualified"])

    def test_r02_consumed_gate_fails_before_any_second_preflight_contact(self) -> None:
