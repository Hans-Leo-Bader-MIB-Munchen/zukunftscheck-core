...
class SemV07QualificationPreRunTests(unittest.TestCase):
    ...
    def test_q03_policy_is_draft_and_requires_explicit_future_model_run_approval(self) -> None:
        policy = json.loads(POLICY.read_text(encoding="utf-8"))
        self.assertEqual(policy["status"], "DRAFT_NOT_HUMAN_APPROVED")
        self.assertTrue(policy["preconditions_for_future_execution"]["explicit_user_model_run_approval_required"])
        self.assertEqual(policy["pass_criteria"]["model_requests_expected"], 16)
        self.assertEqual(policy["pass_criteria"]["parse_success_required"], "16/16")
        self.assertEqual(policy["pass_criteria"]["contract_and_boundary_pass_required"], "16/16")
        self.assertEqual(policy["pass_criteria"]["challenge_cases_pass_required"], "4/4")
    ...
    def test_q06_all_gold_assignments_use_frozen_question_pf_binding(self) -> None:
    ...
    def test_q07_challenge_gold_contains_negative_and_nonconflict_expectations(self) -> None:
        gold = json.loads(GOLD.read_text(encoding="utf-8"))
    ...
    def test_q08_dry_run_manifest_is_strictly_model_free(self) -> None:
    ...
    def test_q09_execute_is_blocked_before_model_contact(self) -> None:
    ...
    def test_q10_runtime_v07_remains_execution_blocked(self) -> None:
