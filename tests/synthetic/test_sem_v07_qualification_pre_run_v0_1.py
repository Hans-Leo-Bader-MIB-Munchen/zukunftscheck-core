...
class SemV07QualificationPreRunTests(unittest.TestCase):
    def test_q01_suite_is_synthetic_and_spans_12_cases(self) -> None:
    ...
    def test_q03_suite_contains_no_expected_gold_keys(self) -> None:
        suite = json.loads(SUITE.read_text(encoding="utf-8"))
        for case in suite["cases"]:
            self.assertFalse(any(key.startswith("expected_") for key in case))

    def test_q04_bundle_validator_requires_all_pf_coverage(self) -> None:
    ...
    def test_q05_each_gold_assignment_uses_frozen_question_pf_binding(self) -> None:
    ...
    def test_q06_dry_run_manifest_is_strictly_model_free(self) -> None:
        ...
        self.assertEqual(manifest["qualification_case_count"], 12)
    ...
    def test_q07_execute_is_blocked_before_model_contact(self) -> None:
    ...
    def test_q08_runtime_v07_remains_execution_blocked(self) -> None:
