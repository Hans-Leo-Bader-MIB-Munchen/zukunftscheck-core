"""Versioned localhost-only structured-output transport for ZS-KI-B smoke tests.

This module keeps the v0.1 transport reproducible and adds a new v0.2 path that
uses the OpenAI-compatible response_format/json_schema mechanism. It performs no
repair, retry, tool use, web access, MCP call or remote transmission.
"""
...
SEMANTIC_SCHEMA_PATH = ROOT / "domains" / "zukunftscheck" / "schema" / "b_semantic_contract_v0_1.schema.json"
RESPONSE_FORMAT_NAME = "zs_ki_b_semantic_response_v0_1"
OUTPUT_MODE_VERSION = "ZS-KI-B-STRUCTURED-OUTPUT-2026-001_v0.2"


def _without_schema_annotations(value: Any) -> Any:
    """Remove non-assertive root/schema annotations while preserving constraints.

    The deterministic semantic boundary remains authoritative. This transport
    schema only constrains model generation to the already frozen contract.
    """
...
def build_response_format() -> dict[str, Any]:
    """Build the strict OpenAI-compatible JSON-schema response_format."""
